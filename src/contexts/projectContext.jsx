...
const myStore = (set) => ({
  ...
  addCardData: async (stage) => {
    ...
    const newCard = {
      ...
      size: {width: 300, height: 200},//added size to card
    };
...
//added setCardSize
  setCardSize: async (cardId, newSize) => {
    set((state) => ({
      ...state,
      cards: state.cards.map((card) =>
        card.uid === cardId ? { ...card, size: newSize } : card
      ),
    }));
  
    const cardDocRef = doc(db, "cards", cardId);
    try {
      await updateDoc(cardDocRef, { size: newSize });
    } catch (error) {
